...
const content = `<template>
  <aside 
    :class="[
      'bg-[#16200B] text-white transition-all duration-300 flex flex-col z-20 shadow-2xl relative',
      isOpen ? 'w-64' : 'w-20'
    ]"
  >
    <!-- Logo Area -->
    <div class="h-16 flex items-center justify-center px-4 border-b border-white/5 bg-[#1b2711]">
      <div class="flex items-center space-x-3 overflow-hidden whitespace-nowrap w-full" :class="isOpen ? 'justify-start' : 'justify-center'">
        ...
        <div v-show="isOpen" class="flex flex-col animate-fade-in">
          <h1 class="font-bold text-lg tracking-wide text-white leading-tight font-serif">NutriSys<span class="text-[#bcdd5a]">.</span></h1>
        </div>
      </div>
    </div>
...
const menus = ref([
  ...
  },
  {
    ...
    items: [
      ...
      { 
        label: 'Skrining & Hitung', 
        icon: 'analytics-outline',
        isOpen: false,
        children: [
          { path: '/calculator/history', label: 'Riwayat Skrining' },
          { path: '/calculator', label: 'Kalkulator Klinis' },
        ]
      }
    ]
  },
  {
    title: 'Inventori Taktis',
    items: [
      { 
        label: 'Bahan Makanan', 
        icon: 'cube-outline',
      ...
      { path: '/stock-opname', label: 'Evaluasi Stok', icon: 'checkmark-done-circle-outline' },
      { path: '/suppliers', label: 'Mitra Pengadaan', icon: 'business-outline' },
    ]
  },
  {
    title: 'SDM & Jadwal',
...
fs.writeFileSync('/Users/wiibs/project/nut/fe/src/components/Sidebar.vue', content);
